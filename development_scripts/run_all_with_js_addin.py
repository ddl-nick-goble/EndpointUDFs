...
OFFICE_ADDIN_HOSTED_URL = os.environ.get("DOMINO_OFFICE_ADDIN_URL", "").strip()
...
# =============================================================================
# Office Add-in Generation Functions (for Excel Online/Mac)
# =============================================================================

def get_project_name(project_id: str) -> str:
    """Get the project name from Domino API or environment variable."""
    # First try environment variable
    project_name = os.environ.get("DOMINO_PROJECT_NAME", "")
    if project_name:
        return clean_function_name(project_name)
    ...
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        name = data.get("name", "DOMINO")
        return clean_function_name(name)
    except Exception:
        return "DOMINO"  # fallback


def generate_office_addin_manifest(endpoints: list[EndpointConfig], hosted_url: str, project_name: str) -> str:
    """Generate Office Add-in manifest XML for Excel Online/Mac."""
    import uuid

    # Generate a unique GUID for this add-in
...
<OfficeApp xmlns="http://schemas.microsoft.com/office/appforoffice/1.1"
  ...
  <DisplayName DefaultValue="Domino Model APIs - {project_name}"/>
  ...
  <!-- Default settings -->
  <DefaultSettings>
    <SourceLocation DefaultValue="{hosted_url}?file=index.html"/>
  </DefaultSettings>
  ...
  <VersionOverrides xmlns="http://schemas.microsoft.com/office/taskpaneappversionoverrides" xsi:type="VersionOverridesV1_0">
      ...
      <Host xsi:type="Workbook">
        <AllFormFactors>
          <ExtensionPoint xsi:type="CustomFunctions">
            <Script>
              <SourceLocation DefaultValue="{hosted_url}?file=functions.js"/>
            </Script>
            <Page>
              <SourceLocation DefaultValue="{hosted_url}?file=index.html"/>
            </Page>
            <Metadata>
              <SourceLocation DefaultValue="{hosted_url}?file=functions.json"/>
            </Metadata>
    ...
    <Resources>
      ...
      <bt:Urls>
        <bt:Url id="Functions.Script.Url" DefaultValue="{hosted_url}?file=functions.js"/>
        <bt:Url id="Functions.Metadata.Url" DefaultValue="{hosted_url}?file=functions.json"/>
        <bt:Url id="Functions.Page.Url" DefaultValue="{hosted_url}?file=index.html"/>
      </bt:Urls>
      <bt:ShortStrings>
...
def generate_office_addin_functions_json(endpoints: list[EndpointConfig], project_name: str) -> str:
    """Generate functions.json metadata for Office.js custom functions."""
    functions = []
    ...
    for ep in endpoints:
        ...
        for p in ep.parameters:
            param_def = {
                ...
                "type": "any"  # Office.js uses "any" for flexible typing
            }
            # Add dimensionality if it's an array parameter
            if p.get("is_array"):
                param_def["dimensionality"] = "matrix"
            params.append(param_def)

        # Create function definition
        # Full name: DOMINO.PROJECTNAME.MODELNAME
        full_name = f"DOMINO.{project_name.upper()}.{ep.name.upper()}"

        func_def = {
            "id": ep.name.upper(),  # Internal ID
            "name": full_name,  # Full hierarchical name shown in Excel
            "description": ep.description,
            "parameters": params,
            "result": {
                "type": "any",
                "dimensionality": "matrix"  # Support array returns (spilling)
            },
            "options": {
                "stream": False,  # Not a streaming function
                "cancelable": False
            }
        }
        functions.append(func_def)

    # Build the complete functions.json
    functions_json = {
        "functions": functions
    }

    return json.dumps(functions_json, indent=2)


def generate_office_addin_functions_js(endpoints: list[EndpointConfig]) -> str:
    """Generate functions.js with JavaScript implementations of custom functions."""

    # Build individual function implementations
    function_impls = []
    ...
    for ep in endpoints:
        ...
        # Build the JSON payload construction
        # We need to handle each parameter and convert to the right type
        data_obj_parts = []
        for p in ep.parameters:
            param_name = p["name"]
            param_type = p.get("type", "string")
            is_array = p.get("is_array", False)

            # For arrays, we pass them as-is; for scalars, we convert based on type
            if is_array:
                data_obj_parts.append(f'        "{param_name}": {param_name}')
            elif param_type == "double":
                data_obj_parts.append(f'        "{param_name}": Number({param_name})')
            elif param_type == "bool":
                data_obj_parts.append(f'        "{param_name}": Boolean({param_name})')
            elif param_type == "date":
                # Convert to yyyy-MM-dd format
                data_obj_parts.append(f'        "{param_name}": formatDateParam({param_name})')
            else:  # string or object
                data_obj_parts.append(f'        "{param_name}": String({param_name})')
 ...
 */
async function {ep.name.lower()}({param_list}) {{
  try {{
    const targetUrl = "{ep.url}";
    const payload = {{
      data: {{
{data_obj}
      }}
...
CustomFunctions.associate("{ep.name.upper()}", {ep.name.lower()});
'''
...
def generate_office_addin_html(endpoints: list[EndpointConfig], project_name: str) -> str:
    """Generate index.html for the Office Add-in task pane."""

    # Build list of functions for display
    func_list_items = []
    for ep in endpoints:
        param_names = ", ".join([p["name"] for p in ep.parameters])
        full_name = f"DOMINO.{project_name.upper()}.{ep.name.upper()}"
        func_list_items.append(f'''
        <li>
          <strong>=<code>{full_name}({param_names})</code></strong>
          <p>{ep.description}</p>
        </li>''')

    func_list_html = "\n".join(func_list_items)
...
<body>
    ...
    <h2>Available Functions ({len(endpoints)})</h2>
    <ul>
{func_list_html}
    </ul>
    ...
    <p><strong>Example:</strong> <code>=DOMINO.{project_name.upper()}.{endpoints[0].name.upper() if endpoints else "MODEL"}(A1, B1)</code></p>
...
def package_office_addin(endpoints: list[EndpointConfig], project_name: str, hosted_url_placeholder: str = "{{HOSTED_URL}}") -> dict[str, str]:
    """
    Package all Office Add-in files into a dictionary.

    Returns dict mapping filename -> content.
    """
    files = {}

    files["manifest.xml"] = generate_office_addin_manifest(endpoints, hosted_url_placeholder, project_name)
    files["functions.json"] = generate_office_addin_functions_json(endpoints, project_name)
    files["functions.js"] = generate_office_addin_functions_js(endpoints)
    files["index.html"] = generate_office_addin_html(endpoints, project_name)

    return files


def generate_static_server_code(addin_files: dict[str, str], endpoints: list[EndpointConfig]) -> str:
    """
    Generate Python Flask app code that serves Office Add-in files via query parameter.

    This creates a model.py that can be deployed as a Domino App or Model API endpoint.
    """

    # Escape the file contents for embedding in Python strings
    def escape_for_python(content: str) -> str:
        return content.replace('\\', '\\\\').replace("'''", "'''")

    # Embed all files as Python string literals
    file_literals = []
    for filename, content in addin_files.items():
        escaped = escape_for_python(content)
        file_literals.append(f"    '{filename}': ('''{escaped}''', get_mimetype('{filename}'))")

    files_dict = ",\n".join(file_literals)

    allowed_urls = ",\n".join([f"    '{ep.url}'" for ep in endpoints])
...
def update_manifest_urls(manifest_content: str, hosted_url: str) -> str:
    """Update placeholder URLs in manifest with actual hosted URL."""
    return manifest_content.replace("{{HOSTED_URL}}", hosted_url)


def create_office_addin_endpoint(project_id: str, addin_files: dict[str, str]) -> str:
    """
    Create a Domino Model API endpoint to host the Office Add-in files.

    NOTE: This is a stub implementation. Full implementation would:
    1. Generate the Flask server code with embedded files
    2. Create a temporary directory and write model.py + requirements.txt
    3. Use Domino API to register a new Model API endpoint
    4. Return the public URL

    For now, we'll print a warning and return a placeholder URL.
    """
    print("  WARNING: Automatic endpoint creation not yet implemented.")
    print("  The Office Add-in files have been generated locally.")
    print("  You'll need to manually deploy them to a web server or Domino App.")
    print()

    # Return a placeholder URL
    # In a real implementation, this would be the actual Domino endpoint URL
    return f"{DOMINO_URL}/models/PLACEHOLDER/latest/model"
...
def main():
    ...
    # Step 3: Generate Office Add-in for Excel Online/Mac
    print()
    print("Step 3: Generating Office Add-in for Excel Online/Mac...")
    print("-" * 40)
    ...
    try:
        ...
        # Package add-in files
        output_dir = os.path.join(os.getcwd(), ".office_addin_build")
        os.makedirs(output_dir, exist_ok=True)

        hosted_url_placeholder = OFFICE_ADDIN_HOSTED_URL or "{{HOSTED_URL}}"
        addin_files = package_office_addin(endpoints, project_name, hosted_url_placeholder)
        print(f"  Generated {len(addin_files)} add-in files:")
        for filename in addin_files.keys():
            print(f"    - {filename}")

        # Resolve the hosted URL (prefer explicit env var)
        if OFFICE_ADDIN_HOSTED_URL:
            hosted_url = OFFICE_ADDIN_HOSTED_URL
            print(f"  Using hosted URL from DOMINO_OFFICE_ADDIN_URL: {hosted_url}")
        else:
            # Create endpoint (stub implementation for now)
            hosted_url = create_office_addin_endpoint(project_id, addin_files)

        # Update manifest content in memory with hosted URL
        addin_files["manifest.xml"] = update_manifest_urls(addin_files["manifest.xml"], hosted_url)
        ...
        # Generate Flask server code (embed updated files)
        server_code = generate_static_server_code(addin_files, endpoints)
        server_path = os.path.join(output_dir, "model.py")
        with open(server_path, "w", encoding="utf-8") as f:
            f.write(server_code)
        print(f"  Generated Flask server: model.py")
        ...
        # Copy manifest to artifacts
        artifacts_dir = "/mnt/artifacts"
        os.makedirs(artifacts_dir, exist_ok=True)
        manifest_path = os.path.join(output_dir, "manifest.xml")
        manifest_artifact = os.path.join(artifacts_dir, "DominoOfficeAddin-manifest.xml")
        shutil.copy(manifest_path, manifest_artifact)

        # Also copy manifest to code root (next to .xll files)
        manifest_code_root = os.path.join(os.getcwd(), "DominoOfficeAddin-manifest.xml")
        shutil.copy(manifest_path, manifest_code_root)

        # Also copy the entire build directory to artifacts
        office_addin_artifact_dir = os.path.join(artifacts_dir, "office_addin")
        if os.path.exists(office_addin_artifact_dir):
            shutil.rmtree(office_addin_artifact_dir)
        shutil.copytree(output_dir, office_addin_artifact_dir)

        print()
        print("=" * 60)
        print("Office Add-in Generated Successfully!")
        print("=" * 60)
        print()
        print("Files location:")
        print(f"  Build directory: {output_dir}")
        print(f"  Manifest (artifacts): {manifest_artifact}")
        print(f"  Manifest (code root): {manifest_code_root}")
        print(f"  Server code: {server_path}")
        print()
        print("To deploy the Office Add-in:")
        print()
        print("OPTION 1: Manual Hosting")
        print(f"  1. Deploy the Flask app in {output_dir} to a web server")
        print("  2. Update the manifest.xml with the actual hosted URL")
        print("  3. Load the manifest in Excel (see instructions below)")
        print()
        print("OPTION 2: Deploy as Domino App")
        print(f"  1. Create a new Domino App from the files in {output_dir}")
        print("  2. Set the app to run: python model.py")
        print("  3. Once deployed, update manifest.xml with the app URL")
        print("  4. Load the updated manifest in Excel")
        print()
        print("=" * 60)
        print("Loading the Add-in in Excel")
        print("=" * 60)
        print()
        print("Excel for Web:")
        print("  1. Open Excel Online (office.com/launch/excel)")
        print("  2. Create or open a workbook")
        print("  3. Click Insert > Office Add-ins > Upload My Add-in")
        print(f"  4. Upload: {manifest_artifact}")
        print()
        print("Excel for Mac:")
        print("  1. Open Excel for Mac")
        print("  2. Go to Insert > Add-ins > My Add-ins")
        print("  3. Click 'Upload My Add-in'")
        print(f"  4. Select: {manifest_artifact}")
        print()
        print("Centralized Deployment (Admin):")
        print("  1. Go to Microsoft 365 Admin Center")
        print("  2. Navigate to Settings > Integrated apps")
        print("  3. Click 'Upload custom apps'")
        print(f"  4. Upload: {manifest_artifact}")
        print("  5. Assign to users/groups")
        print()
        print("=" * 60)
        print(f"Available Functions ({len(endpoints)} total)")
        print("=" * 60)
        print()
        for ep in endpoints:
            params = ", ".join([p["name"] for p in ep.parameters])
            full_name = f"DOMINO.{project_name.upper()}.{ep.name.upper()}"
            print(f"  ={full_name}({params})")
            print(f"    {ep.description}")
            print()

        print("Note: Remember to update the manifest.xml with your actual hosted URL")
        print(f"      before loading it in Excel. Current URL is a placeholder.")
        print()

    except Exception as e:
        print(f"\nOffice Add-in Generation Error: {e}")
        print("Excel-DNA add-in was generated successfully.")
        print("Office Add-in generation can be skipped for now.")
        import traceback
        traceback.print_exc()
